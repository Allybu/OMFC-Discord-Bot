...
const { WowApi } = require('./wowApi');

WowApi.init().catch((err) =>
    console.error('Could not connect to wow API', err)
);
...
exports.Roster = {
    getRaiderIoForChar: async (character, realm) => {
        try {
            const { data } = await axios.get(
                `https://raider.io/api/v1/characters/profile?region=EU&realm=${realm}&name=${character}&fields=gear,guild,raid_progression,mythic_plus_ranks,mythic_plus_scores_by_season:current`
            );
            return data;
        } catch (error) {
    ...
    getWarcraftlogsData: async (
        ...
        console.log(
            `Get Warcraftlogs Data For: ${character} - ${specName} - ${difficulty}`
        );
        ...
        if (members) {
            await asyncForEach(members, async (m) => {
                if (m.rank <= 2) {
                    ...
                    // get discord member
                    const discordGuildMember = await guild.members.cache.find(
                        (member) => {
                            return member.nickname
                                ? member.nickname.includes(char.name)
                                : false;
                        }
                    );
                    ...
                    if (discordGuildMember) {
                        try {
                            ...
                            realName = discordGuildMember.nickname
                                .split('(')[1]
                                .split(')')[0];
                        } catch (e) {
                            console.warn(e);
                    ...
                    await asyncForEach(m.specs, async (spec) => {
                        ...
                        const warcraftLogsData = await this.Roster.getWarcraftlogsData(
                            char.name,
                            char.realm.slug,
                            specName,
                            metric,
                    ...
                    const raiderIoData = await this.Roster.getRaiderIoForChar(
                        char.name,
                        char.realm.slug
                    );
                    if (!raiderIoData) {
                    ...
                    const character = await WowApi.getCharacter(
                        char.name,
                        char.realm.slug
                    );
                    const characterMedia = await WowApi.getCharacterMedia(
                        char.name,
                        char.realm.slug
                    );
                    ...
                    const image = characterMedia.assets.find(
                        (a) => a.key === 'avatar'
                    ).value;
                    const bigImage = characterMedia.assets.find(
                        (a) => a.key === 'inset'
                    ).value;
                    ...
                    if (character.covenant_progress) {
                        covenant.level =
                            character.covenant_progress.renown_level;
                        covenant.name =
                            character.covenant_progress.chosen_covenant.name;
                    }
                    ...
                    roster.push({
                        ...
                        itemLevelCombined,
                        mythicScore:
                            raiderIoData.mythic_plus_scores_by_season[0].scores
                                .all,
                        mythicRanks: raiderIoData.mythic_plus_ranks,
                        raidSummery: raiderIoData.raid_progression[
                            'castle-nathria'
                        ]
                            ? raiderIoData.raid_progression['castle-nathria']
                                  .summary
                            : null,
