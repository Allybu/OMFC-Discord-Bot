...
const setNewRoster = async (client) => {
    const channel = client.channels.cache.find(channel => channel.name.includes('roster'));
    if (channel) {
        ...
        members.forEach(member => {
            const user = member.discordGuildMember ? member.discordGuildMember.user : null;
            const discordImage = user ? `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png` : null;

            let mythicScore = '';

            // const healEmoji = client.emojis.get("name", "roleheal");
            if (member.mythicRanks.class_tank && member.mythicRanks.class_tank.realm > 0) {
                mythicScore += ` <:roletank:748830899541639188>${getInline(member.mythicRanks.class_tank.realm)}`;
            }
            if (member.mythicRanks.class_dps && member.mythicRanks.class_dps.realm > 0) {
                mythicScore += ` <:roledd:748831003954511872>${getInline(member.mythicRanks.class_dps.realm)}`;
            }
            if (member.mythicRanks.class_healer && member.mythicRanks.class_healer.realm > 0) {
                mythicScore += ` <:roleheal:748830972086059088>${getInline(member.mythicRanks.class_healer.realm)}`;
            }
            ...
            if (member.warcraftLogs) {
                ...
                member.warcraftLogs.forEach(specLogs => {
                    ...
                    allStars.push({
                        spec: specLogs.spec,
                        percent: Math.round(specLogs.data.zoneRankings.bestPerformanceAverage*10)/10,
                    });
                });
                ...
                if (allStars.length) {
                    const maxAllPercent = Math.max.apply(Math, allStars.map(r => r.percent));
                    const maxAllRanking = allStars.find(r => r.percent === maxAllPercent);
                    warcraftLogsAllStar = `${getLogColor(maxAllRanking.percent)}${getInline(maxAllRanking.percent)}`;
                }
                ...
                encounterOrder.forEach(e => {
                    if (encounters[e]){
                        ...
                        const maxPercent = Math.max.apply(Math, rankings.map(r => r.percent));
                        const maxRanking = rankings.find(r => r.percent === maxPercent);

                        warcraftLogsRankings += `\n${e}: ${getLogColor(maxRanking.percent)}${getInline(maxRanking.percent)} (${maxRanking.spec})`;
            ...
            try {
                let description =  `${getEmojiByClass(member.className)} ${member.race} ${member.spec} ${member.className}`;
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .addField("iLvl", `${getInline(member.itemLevelCombined)}`, true)
                    .addField("M+ Score", `${getInline(member.mythicScore)}`, true);
    
                if (warcraftLogsAllStar.length) {
                    embed.addField('Log Rank', warcraftLogsAllStar, true);
                }

                if (member.covenant && member.covenant.level && member.covenant.name) {
                    embed.addField('Covenant', `${member.covenant.name} (${member.covenant.level})`, true);
                }
    
                embed.addField("Progress", member.raidSummery, true)
                    .addField("Archievements", member.archievements, true);

                if (mythicScore.length) {
                    embed.addField(`Mythic+ Class Realm Rank`, mythicScore, false);
                }
               
                if (warcraftLogsRankings.length) {
                    embed.addField('Encounter Rankings (Heroic)', warcraftLogsRankings, false);
                }
...
const setInvites = async (client, message) => {
    const channel = client.channels.cache.find(channel => channel.name.includes(inviteChannel));
    if (channel) {
...
const sendInviteLog = async (options) => {
    ...
    const content = options.originalMessage.content;
    if (content.includes(':clock')) {
        ...
        options.originalMessage.reactions.cache.each(r => {
            const role = printRole(r.emoji.name);
            if (role) {
...
const listenForInviteReactions = async (client) => {
    const channel = client.channels.cache.find(channel => channel.name.includes(inviteChannel));
    const logChannel = client.channels.cache.find(channel => channel.name.includes(invitelogs));
    if (channel && logChannel) {
        await channel.messages.fetch();
        client.on('messageReactionAdd', async (reaction, user) => {
            if (!user.bot && reaction.message.channel.name.includes(inviteChannel)) {
                sendInviteLog({
